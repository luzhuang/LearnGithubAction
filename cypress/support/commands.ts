...
Cypress.Commands.add(
  "screenShotAfterRequest",
  (category, name, request, deltaTime = 100, threshold = 0, delay = 1000) => {
...
Cypress.Commands.add("screenShotPauseAfterRequest", (category, name, request, deltaTime = 100, delay = 1000) => {
  cy.intercept("GET", request).as("initialRequest");
  cy.visit(`/mpa/${name}.html`);
  cy.window().then((win) => {
    win.Math.random = () => 0.5;
